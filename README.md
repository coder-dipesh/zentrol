...
- **`vercel.json`** runs **`pip install -r requirements-vercel.txt`** — no PyTorch/Lip2Speech/**PyMuPDF** by default (bundle limit); **`algoliasearch`** is included for ⌘K search when **`ALGOLIA_*`** is set.
...
- **PostgreSQL `DATABASE_URL` is required** (SQLite is rejected on serverless). **`vercel.json`** runs **`migrate`** when the platform honors **`buildCommand`**; **`config/serverless_db.py`** also migrates on cold start (disable with **`SKIP_SERVERLESS_STARTUP_MIGRATE=True`** only if migrations always run elsewhere). **`MEDIA_ROOT`** uses **`/tmp`** on serverless; dashboard **`.pptx`** uploads are parsed in the browser then saved as JSON (binary **`.ppt`** unsupported there).
